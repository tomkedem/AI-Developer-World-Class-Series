---
title: "פרק 11 – דקורטורים, Context Managers ו-dataclass"
weight: 12
---
...
בלי לגעת בקוד שלה.
למשל: 
להדפיס הודעות, למדוד זמן ריצה או לבדוק הרשאות.
במקום לשכפל את אותה לוגיקה שוב ושוב, 
משתמשים ב-**דקורטור**.

**הרעיון פשוט**
דקורטור הוא **פונקציה שעוטפת פונקציה אחרת**.
מוסיפה לה קוד לפני ואחרי,
ומחזירה פונקציה חדשה.  
...
say_hello()
```
שימוש ב-@wraps שומר על שם הפונקציה וה-docstring המקוריים,
כדי שלא נאבד מידע חשוב על הפונקציה המקורית. 
...
אם אין צורך במחלקה שלמה, אפשר להשתמש בדקורטור @contextmanager
...
with open_utf8("data/test.txt", "w") as f:
    f.write("High quality Hebrew text")
```
...
דקורטורים, Context Managers ו-dataclass אינם קסמים, הם פשוט תחביר שמאפשר **לחשוב ברמה גבוהה יותר**.
הם מסירים חזרות, מנהלים משאבים בביטחון, ומשאירים את הלוגיקה העסקית ממוקדת.
כשאתה משתמש בהם נכון, הקוד שלך נראה פחות כמו רצף של פעולות,
