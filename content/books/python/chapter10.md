---
title: "פרק 10 – טיפוסיות סטטית עם typing"
weight: 11
---
...
- `a: int` אומר שהפרמטר `a` צריך להיות מספר שלם  
- `-> int` אומר שהפונקציה מחזירה מספר שלם  
- שימוש ב-`| None` או `Optional` מציין שהפונקציה עשויה להחזיר גם `None`
...
```python
et_mode("train")  # valid
set_mode("debug")  # error not one of the allowed modes
```
**Literal – ערכים קבועים בלבד**  

Literal מאפשר להגביל פרמטר לערכים ספציפיים בלבד.
זה שימושי במיוחד כשיש פונקציה שפועלת בכמה **מצבים קבועים** מראש.
```python
...
-  עוזר לעורכים (כמו VS Code או PyCharm) להציע ערכים נכונים בלבד.
-  מונע באגים טיפשיים עוד לפני שהקוד רץ.
-  גם תיעוד מצוין, כל מי שקורא את הפונקציה מבין מיד מה הערכים האפשריים ומה היא מחזירה.
...
```
כך כלים כמו mypy יכולים לדעת ש-words חייב להיות רשימה של מחרוזות,
ושגיאה תתגלה מוקדם יותר, עוד לפני שהקוד רץ.

## TypedDict – מבנה דמוי אובייקט מוגדר
כשאנחנו עובדים עם dictionaries שמתארים מבנה קבוע (למשל תגית של dataset),
נרצה לתעד בדיוק אילו שדות קיימים ואילו סוגים הם מחזיקים.
```python
...
במילים אחרות, לא משנה מאיזו מחלקה האובייקט הגיע,
כל עוד יש לו את המתודות שהקוד שלך מצפה להן, זה מספיק.
```python
class Dog:
    def speak(self):
        print("Woof!")
...
או תעביר אובייקט שאין לו את המתודה הדרושה
mypy יתריע על כך עוד לפני ההרצה.
...
בכל מקום שבו הקוד “דיבר בעמימות”, כעת יש הגדרה ברורה.
גם המפתח הבא יוכל להבין בדיוק אילו סוגים נכנסים ויוצאים מכל פונקציה.
## Best Practices
...
ומאפשרים למערכות גדולות, כמו פרויקטי AI מבוזרים 
לשמור על עקביות גם כשהן מתפתחות במהירות.
פייתון תישאר שפה דינמית, אבל עם typing נכון 
היא הופכת לשפה **אמינה, מתועדת ומובנת הרבה יותר**.
