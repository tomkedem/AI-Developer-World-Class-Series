...
כדי לחזור למצב רגיל, כתוב בטרמינל:
```text
deactivate
```
**טיפ קטן**  
...
```text
poetry install   # installs everything according to pyproject.toml and poetry.lock
poetry shell     # enters the project virtual environment
```
...
version = "0.1.0"
description = "Short text cleaning and analysis"
...
## yproject.toml מול requirements.txt: מתי לבחור במה
...
def main() -> None:
    text = "Python is a great language for smart text processing"
    print(analyze(text))
...
- **מכניסים**: קוד מקור, pyproject.toml ו-poetry.lock או requirements.txt, קונפיגים, סקריפטים, דוקומנטציה.
- **לא מכניסים:**
