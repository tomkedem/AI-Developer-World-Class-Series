---
title: "פרק 3 – מבני נתונים שימושיים"
weight: 4
---
...
ולראות איך להשתמש בהם **כמו מהנדסים**, לא כמו חובבנים.
...
data.insert(0, 5)  # inserts at the beginning
data.remove(30)    # removes a specific value
...
אם הרשימה היא לב המערכת,
המילון (dict) הוא המוח שלה.
זהו מבנה הנתונים שבו פייתון באמת מצטיינת 
מיפוי של מפתח לערך, כמו טבלה קטנה בזיכרון,
עם גישה מיידית לכל נתון בלי צורך לעבור על כל הרשימה.
**איך נראֶה מילון**
```python
person = {
    "name": "תמר",
...
**הוספה, עדכון והסרה**  
```python
person["city"] = "תל אביב"    # add
person["age"] = 30           # update
del person["is_active"]      # delete
```
פשוט, ישיר, וקריא.
...
כך נראים כמעט כל הפלטים שמוחזרים
 מ-OpenAI, Hugging Face, או LangChain.
...
print("a" in data)         # check if key exists
print(data.keys())         # show all keys
print(data.values())       # show all values
print(list(data.items()))  # show list of (key, value) pairs
```
...
print(a ^ b)  # symmetric difference (values unique to each side)
...
לעומת זאת, אם העדיפות היא למהירות ולייחודיות 
set ינצח בכל פעם.
  
  **טיפ הנדסי**  

במערכות AI, חישובים רבים מסתמכים על זיהוי חפיפות, כפילויות וייחודיות.  
למשל, כמה מילים חדשות הופיעו בטקסט,
או כמה מזהים שונים עברו בתהליך מסוים.
set הוא מבנה הנתונים המושלם לזה:
...
אם list היא רשימה גמישה ודינמית,
tuple הוא ההפך המוחלט. 
...
```python
point[0] = 5  # ❌ will raise an error: 'tuple' object does not support item assignment
```
...
```python
data = (1, 2, 3)
single = (5,)        # yes, even a single-element tuple needs a comma!
as_list = list(data)
...
```python
array.shape  # for example returns (3, 5)
```
...
tuple נועד ליציבות.
במערכות AI, זה שימושי במיוחד כשמעבירים נתונים בין שלבים ב-pipeline 
...
queue.append("task4")       # add to the end
queue.appendleft("urgent")  # add to the beginning
...
במערכות שבהן יש תורים (queues) או זרימת נתונים (streams),
deque הוא הבחירה הנכונה.
...
לספור מילים, לחשב ממוצע אורך, ולמצוא את המילה הנפוצה ביותר.
זו אותה לוגיקה שמופיעה כמעט בכל שלב של עיבוד שפה טבעית (NLP).
