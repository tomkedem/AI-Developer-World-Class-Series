---
title: "פרק 9 – תכנות מונחה עצמים בסיסי"
weight: 10
---
...
בפייתון, מחלקה נוצרת בעזרת מילת המפתח class.
המתודה __init__ היא הפונקציה שמופעלת בכל פעם שנוצר מופע חדש של המחלקה, ו-self מייצגת את האובייקט הנוכחי.
```python
class Counter:
...
class Vector:
    def __init__(self, x: int, y: int):
        self.x = x      # Store X value
        self.y = y      # Store Y value

    def __str__(self) -> str:
        return f"({self.x}, {self.y})"   # User-friendly output

    def __repr__(self) -> str:
        return f"Vector(x={self.x}, y={self.y})"   # Developer-focused output

    def __len__(self) -> int:
        return abs(self.x) + abs(self.y)  # Custom length definition
...
- `__repr__` נותן ייצוג מדויק שמתאים למפתחים ולדיבוג.  
- `__len__` מאפשר להפעיל `len(obj)` כאילו זה אוסף עם אורך מוגדר.  

השילוב של המתודות האלה הופך את המחלקה לנוחה לשימוש בפייתון, הרבה יותר ממחלקה רגילה בלי ההגדרות האלה.
...
לכל מחלקה יכולים להיות  **משתני מחלקה  (class attributes)**   ערכים שחלים על כל המופעים.
```python
class Tokenizer:
    language = "hebrew"  # Class variable shared by all instances

    def __init__(self, text: str):
        self.text = text  # Instance variable unique to each object
...
class User:
    ...
    def greet(self):
        print(f"היי {self.name}!")  # Print a greeting in Hebrew
...
אבל אם תשים אותה בתוך המחלקה, היא נשארת קרובה ונגישה, וזה עושה סדר.
...
def apply_vat(amount): ...
```
היא הייתה עובדת בדיוק אותו דבר 
אבל היא הייתה “תלושה” מהקוד הלוגי של ההזמנות.  
@staticmethod עוזרת לשמור על ארגון טוב בקוד:
פונקציות שקשורות לנושא מסוים נשארות יחד,
גם אם הן לא תלויות במחלקה.  
...
p.x = 10          # ❌ this will raise an error at runtime
```
הפלט בפועל יהיה:
```python
Point(x=3, y=4)
...
```
כשאתה כותב `@dataclass(order=True)`, פייתון מייצרת אוטומטית את כל מתודות ההשוואה:
...
ברירת המחדל היא שההשוואה מתבצעת לפי **סדר השדות** שהוגדרו במחלקה.
...
class Cleaner:
    def clean(self, text: str) -> str:
        return text.lower().strip()  # Convert to lowercase and remove surrounding spaces
...
class TextCleaner:
    ...
    def clean(self, text: str) -> str:
        ...
        return result.strip()  # Remove surrounding spaces

# Usage
cleaner = TextCleaner(lower=True)
print(cleaner.clean("!שלום, עולם!!!"))  # שלום עולם
```
היתרונות ברורים:
- קוד קריא.
